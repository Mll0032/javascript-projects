...
function reverseCharacters(str) {
    ...
    fullString = str.split('').reverse().join('');
    } else { (typeof str === String)
    fullString = str.toString().split('').reverse().join('');
    }
    return fullString;
}
...
function completeReverse(arr){
    arrHold = [];
    for (let i = 0; i < arr.length; i++) {
        arrHold.push(reverseCharacters(arr[i]));
}
return arrHold;
}
console.log(completeReverse(arrayTest1));
// Bonus Missions
...
let length = 2;
let width = 4;
console.log(`The area is ${calculateArea(length, width)} cm^2.`);

length = 20;
console.log(`The area is ${calculateArea(length)} cm^2.`);